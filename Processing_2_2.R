...
# read cel files
setwd('/Users/dorimersch/Desktop/Dorothee/2_affy_chips01')
celfiles <- list.files(pattern = "\\.CEL$")
raws_cel <- read.celfiles(celfiles)

# read phenoData
pdat <- read_excel("~/Desktop/Dorothee/RNA_for_HD_markiert_Saskia_sorted_by_color.xlsx", sheet = 3)
...
df_PCA_raw <- data.frame(PCA_raw$x[,1:5],pd@data)
...
pdf(paste("./Figures/EXP_2_FIGURE_PCA_no_correction.pdf",sep = ""),width = 12, height = 8)
...
df_PCA_rma <- data.frame(PCA_rma$x[,1:5],pd@data)
df_PCA_rma$date_1 <- as.factor(df_PCA_rma$date_1)
df_PCA_rma$date_2 <- as.factor(df_PCA_rma$date_2)

pdf(paste("./Figures/EXP_2_FIGURE_PCA_after_correction_rma.pdf",sep = ""),width = 12, height = 8)
for (variable in colnames(pdat_filtered)[-2]) {
  for (PCs in list_PCs) {
...
# use updated anova function to compute pov
PoV_PCs_list = list()
for (variable in colnames(pdat_filtered)[-2]) {
...
#DGE
# Design matrix
## according to PoV, date_1, date_2 and expirementator do not explain any variance, however they seem to be dependent from each other, i will nevertheless correct for date_1
####### NOTE! only 2 dates, 2 expirementators, T-TEST instead of ANOVA NECCESSARY!!!!
...
contrast_matrix <- makeContrasts(IL4I1=cond_U251_Il4I1-cond_U251,
                                 levels = design_eset)
## DGE 2sd
...
fit_2sd_eB <- eBayes(fit_2sd_cont, robust = T) #trend = T,
saveRDS(fit_2sd_eB, "./RDS/fit_2sd_eB_EXP2.rds")
...
walk2(list(tt_2sd_u_AHR), paste("./TopTables/tt_2sd_","IL4I1","_AHR.txt",sep = ""), write.table, quote = F, sep = "\t")
names(tt_2sd_u_AHR) <- "IL4I1"

## Barcodeplots AHR signature
index_1 <- tt_2sd_u_AHR$IL4I1$hGene%in%AHR_genes$Gene

pdf("./Figures/barcodeplot_EXP_2.pdf")
barcodeplot(tt_2sd_u_AHR$IL4I1$t,index_1, main="IL4I1 in U251 PBMC")
dev.off()
...
write.table(all_roast, "./GSA/AHR_enrichment_IL4I1_all.txt", row.names = F, sep = "\t")

gsa_res <- gsa_FUN2(top_t = tt_2sd_u_AHR$IL4I1, rma_obj = rma_2sd, exprmnt = "Exp_2", cntrst = "many",
...
walk(1:length(gsa_res), safely(gsa_bar_plot_FUN), gsa_ls=gsa_res, res_path="/Figures/",
     coi="EXP_2_IL4I1", wd=12, ht=8, file_type="pdf")


gsa_paths <- list.files("./GSA/",pattern = "Exp_2",full.names = T)
...
condition_names<-gsub("gsa_|_Exp_2.*.","",gsa_paths_names)
...
## Volcano plots
tt_IL4I1 <- tt_AHR_FUN(tt_2sd_u_AHR$IL4I1, AHR_genes$Gene, 0.58, 2)
v_p_IL4I1 <- volc_plot(tt_IL4I1)

pdf("./Figures/EXP2_IL4I1_volcano_plot.pdf", width = 8, height = 8, pointsize = 28)
v_p_IL4I1
dev.off()
