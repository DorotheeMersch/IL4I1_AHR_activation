...
# read cel files
setwd('/Users/dorimersch/Desktop/Dorothee/4_affy_chips01')
celfiles <- list.files(pattern = "\\.CEL$")
raws_cel <- read.celfiles(celfiles)

# read phenoData
pdat <- read_excel("~/Desktop/Dorothee/RNA_for_HD_markiert_Saskia_sorted_by_color.xlsx", sheet = 5)
...
pdat$donor <- gsub("_[^_]+$","",pdat$LINA) %>% ifelse(nchar(.)==0,"unknown",.)%>%gsub("_Spender ","",.)%>%gsub("_CD8","",.)%>%gsub("_","",.)
...
pdf(paste("./Figures/EXP_4_FIGURE_PCA_no_correction.pdf",sep = ""),width = 12, height = 8)
...
pdf(paste("./Figures/EXP_4_FIGURE_PCA_after_correction_rma.pdf",sep = ""),width = 12, height = 8)
for (variable in colnames(pdat_filtered)[-2]) {
  for (PCs in list_PCs) {
...
saveRDS(fit_2sd_eB, "./RDS/fit_2sd_eB_EXP4.rds")
...
pdf("./Figures/barcodeplot_EXP_4.pdf")
barcodeplot(tt_2sd_u_AHR$IL4I1$t,index_1, main="IL4I1 in CD8 U251 PBMC")
dev.off()
...
gsa_res <- gsa_FUN2(top_t = tt_2sd_u_AHR$IL4I1, rma_obj = rma_2sd, exprmnt = "Exp_4", cntrst = "many",
...
walk(1:length(gsa_res), safely(gsa_bar_plot_FUN), gsa_ls=gsa_res, res_path="/Figures/",
     coi="EXP_4_IL4I1", wd=12, ht=8, file_type="pdf")


gsa_paths <- list.files("./GSA/",pattern = "Exp_4",full.names = T)
...
condition_names<-gsub("gsa_|_Exp_4.*.","",gsa_paths_names)
...
pdf("./Figures/EXP4_IL4I1_volcano_plot.pdf", width = 8, height = 8, pointsize = 28)
v_p_IL4I1
dev.off()
