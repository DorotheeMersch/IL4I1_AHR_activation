...
# read cel files
setwd('/Users/dorimersch/Desktop/Dorothee/6_affy_chips01')
celfiles <- list.files(pattern = "\\.CEL$")
raws_cel <- read.celfiles(celfiles)

# read phenoData
pdat <- read_excel("~/Desktop/Dorothee/RNA_for_HD_markiert_Saskia_sorted_by_color.xlsx", sheet = 7, range = "A1:S9")
...
df_PCA_raw <- data.frame(PCA_raw$x[,1:8],pd@data)
...
pdf(paste("./Figures/EXP_6_FIGURE_PCA_no_correction.pdf",sep = ""),width = 12, height = 8)
...
df_PCA_rma <- data.frame(PCA_rma$x[,1:8],pd@data)
df_PCA_rma$date_1 <- as.factor(df_PCA_rma$date_1)
df_PCA_rma$date_2 <- as.factor(df_PCA_rma$date_2)

pdf(paste("./Figures/EXP_6_FIGURE_PCA_after_correction_rma.pdf",sep = ""),width = 12, height = 8)
for (variable in colnames(pdat_filtered)[-2]) {
  for (PCs in list_PCs) {
...
# updated anova function to compute pov (from functions_processing.R)
PoV_PCs_list = list()
for (variable in colnames(pdat_filtered)[-2]) {
...
saveRDS(fit_2sd_eB, "./RDS/fit_2sd_eB_EXP6.rds")
...
pdf("./Figures/barcodeplot_EXP_6.pdf")
barcodeplot(tt_2sd_u_AHR$IL4I1$t,index_1, main="IL4I1 in CD4 U251 PBMC")
dev.off()


#### GSA ####
# use -log pval instead of the score
...
gsa_res <- gsa_FUN2(top_t = tt_2sd_u_AHR$IL4I1, rma_obj = rma_2sd, exprmnt = "Exp_6", cntrst = "many",
...
walk(1:length(gsa_res), safely(gsa_bar_plot_FUN), gsa_ls=gsa_res, res_path="/Figures/",
     coi="EXP_6_IL4I1", wd=12, ht=8, file_type="pdf")


gsa_paths <- list.files("./GSA/",pattern = "Exp_6",full.names = T)
...
condition_names<-gsub("gsa_|_Exp_6.*.","",gsa_paths_names)
...
pdf("./Figures/EXP6_IL4I1_volcano_plot.pdf", width = 8, height = 8, pointsize = 28)
v_p_IL4I1
dev.off()