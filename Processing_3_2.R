...
# read cel files
setwd('/Users/dorimersch/Desktop/Dorothee/3_affy_chips01')
celfiles <- list.files(pattern = "\\.CEL$")
raws_cel <- read.celfiles(celfiles)

# read phenoData
pdat <- read_excel("~/Desktop/Dorothee/RNA_for_HD_markiert_Saskia_sorted_by_color.xlsx", sheet = 4, range = "A1:S13")
...
pdat$donor <- gsub("_CD8.*","",pdat$LINA) %>% ifelse(nchar(.)==0,"unknown",.)%>%gsub("_Spender ","",.)
...
pdf(paste("./Figures/EXP_3_FIGURE_PCA_no_correction.pdf",sep = ""),width = 12, height = 8)
...
pdf(paste("./Figures/EXP_3_FIGURE_PCA_after_correction_rma.pdf",sep = ""),width = 12, height = 8)
for (variable in colnames(pdat_filtered)[-2]) {
  for (PCs in list_PCs) {
...
contrast_matrix <- makeContrasts(HPP=cond_40uM_HPP-cond_005_DMSO, I3P=cond_3uM_I3P-cond_005_DMSO,
                                 levels = design_eset)
## DGE 2sd
...
saveRDS(fit_2sd_eB, "./RDS/fit_2sd_eB_EXP3.rds")
...
index_1 <- tts_2sd_u_AHR$HPP$hGene%in%AHR_genes$Gene
index_2 <- tts_2sd_u_AHR$I3P$hGene%in%AHR_genes$Gene

pdf("./Figures/barcodeplot_EXP_3.pdf")
barcodeplot(tts_2sd_u_AHR$HPP$t,index_1, main="40uM HPP in CD8 PBMC")
barcodeplot(tts_2sd_u_AHR$I3P$t,index_2, main="3uM I3P in CD8 PBMC")
dev.off()
...
write.table(all_roast, "./GSA/AHR_enrichment_HPP_I3P_all.txt", row.names = F, sep = "\t")

gsa_res <- gsa_FUN2(top_t = tts_2sd_u_AHR$HPP, rma_obj = rma_2sd, exprmnt = "Exp_3", cntrst = "many",
...
gsa_paths <- list.files("./GSA/",pattern = "Exp_3",full.names = T)
...
condition_names<-gsub("gsa_|_Exp_3.*.","",gsa_paths_names)
...
pdf("./Figures/EXP3_I3P_volcano_plot.pdf", width = 8, height = 8, pointsize = 28)
v_p_I3P
dev.off()
...
pdf("./Figures/EXP3_HPP_volcano_plot.pdf", width = 8, height = 8, pointsize = 28)
v_p_HPP
dev.off()
