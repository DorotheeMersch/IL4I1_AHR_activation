...
# read cel files
setwd('/Users/dorimersch/Desktop/Dorothee/1_affy_chips01')
celfiles <- list.files(pattern = "\\.CEL$")
raws_cel <- read.celfiles(celfiles)

# read phenoData
pdat <- read_excel("~/Desktop/Dorothee/RNA_for_HD_markiert_Saskia_sorted_by_color.xlsx", sheet = 2, range = "A1:S13")
...
pdat$donor <- gsub("_[^_]+$","",pdat$LINA) %>% ifelse(nchar(.)==0,"unknown",.)%>%gsub("_Spender ","",.)%>%paste0("donor_",.)
...
pdf(paste("./Figures/EXP_1_FIGURE_PCA_no_correction.pdf",sep = ""),width = 7, height = 5)
for (variable in colnames(pdat_filtered)[-2]) {
  for (PCs in list_PCs) {
    print(ggscatter(df_PCA_raw, PCs[1],PCs[2],color=variable)+theme_pubr()+theme(text = element_text(size=15,face="bold")))
  }
}
dev.off()
...
pdf(paste("./Figures/EXP_1_FIGURE_PCA_after_correction_rma.pdf",sep = ""),width = 12, height = 8)
for (variable in colnames(pdat_filtered)[-2]) {
  for (PCs in list_PCs) {
...
# updated anova function to compute pov (functions_processing.R)
PoV_PCs_list = list()
for (variable in colnames(pdat_filtered)[-2]) {
...
# chisq_pval from functions_processing.R
...
contrast_matrix <- makeContrasts(I3P=cond_25uM_I3P-cond_DMSO,
                                 levels = design_eset)
## DGE 2sd
...
saveRDS(fit_2sd_eB, "./RDS/fit_2sd_eB_EXP1.rds")

## all tts
tt_2sd_all <- topTable(fit_2sd_eB, number = Inf,adjust.method = "BH")
tt_2sd_u_all <- tt_2sd_all[,c(29,25,40:45)]
#names(tt_2sd_u_all) <- colnames(contrast_matrix)

## annotation update
require(dplyr)
tt_2sd_u_all <- annot_hgcn_FUN2(tt_2sd_u_all, hgnc = hgnc)
...
walk2(list(tt_2sd_u_AHR), paste("./TopTables/tt_2sd_","I3P","_AHR.txt",sep = ""), write.table, quote = F, sep = "\t")
names(tt_2sd_u_AHR) <- "I3P"

## Barcodeplots AHR signature
index_1 <- tt_2sd_u_AHR$I3P$hGene%in%AHR_genes$Gene

pdf("./Figures/barcodeplot_EXP_1.pdf")
barcodeplot(tt_2sd_u_AHR$I3P$t,index_1, main="25 uM I3P in PBMC")
dev.off()


##### from here on analysis if we would take DMSO and Medium separately as control #####
## all tts
...
## Barcodeplots AHR signature
#index_1 <- tts_2sd_u_AHR$I3P_DMSO$hGene%in%AHR_genes$Gene
#index_2 <- tts_2sd_u_AHR$I3P_Medium$hGene%in%AHR_genes$Gene

#pdf("./Figures/barcodeplot_EXP_1.pdf")
#barcodeplot(tts_2sd_u_AHR$I3P_DMSO$t,index_1, main="I3P_DMSO")
#barcodeplot(tts_2sd_u_AHR$I3P_Medium$t,index_2, main="I3P_Medium")
#dev.off()
...
all_roast <- map2(tt_2sd_u_AHR, as.data.frame(contrast_matrix), roast_FUN, gs=AHR_genes$Gene, rma_obj=rma_2sd, des_mat=design_eset, arr_wts=arr_wts, block = donor, correlation = corfit$cor) %>% do.call(rbind,.)
all_roast <- data.frame(Condition=rownames(all_roast), all_roast, stringsAsFactors = F)

write.table(all_roast, "./GSA/AHR_enrichment_I3P_all.txt", row.names = F, sep = "\t")

gsa_res <- gsa_FUN2(top_t = tt_2sd_u_AHR$I3P, rma_obj = rma_2sd, exprmnt = "Exp_1", cntrst = "many",
                   cnt_mat = contrast_matrix, res_path = "./GSA/",wts = arr_wts,
                   msig.data.lists = msig.data.lists, msigs = 14, pltfrm = "Affy", d_m = design_eset)

#names(gsa_res) <- names(tt_2sd_u_AHR)

walk(1:length(gsa_res), safely(gsa_bar_plot_FUN), gsa_ls=gsa_res, res_path="/Figures/",
     coi="EXP_1_I3P", wd=12, ht=8, file_type="pdf")


gsa_paths <- list.files("./GSA/",pattern = "Exp_1",full.names = T)
...
condition_names<-gsub("gsa_|_Exp_1.*.","",gsa_paths_names)
...
## Volcano plots
tt_I3P <- tt_AHR_FUN(tt_2sd_u_AHR$I3P, AHR_genes$Gene, 0.58, 2)
v_p_I3P <- volc_plot(tt_I3P)

pdf("./Figures/EXP1_I3P_volcano_plot.pdf", width = 8, height = 8, pointsize = 28)
v_p_I3P
dev.off()
